...
function setQuaternionFromProperEuler(q: any, a: number, b: number, c: number, order: string) {
    const cos = Math.cos;
    const sin = Math.sin;
...
function denormalize(value: number, array: any): number {
...
function normalize(value: number, array: any): number {
    switch (array.constructor) {
        case Float32Array:
            return value;
        case Uint16Array:
